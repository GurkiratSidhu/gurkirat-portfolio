import React from "react";

function Experience() {
	return (
		<div>
			{" "}
			<div className="my-10 text-left md:mx-10 dark:text-white">
				<h1 className="mb-4 text-3xl font-extrabold text-left md:text-4xl">
					Experience
				</h1>

				<h2 className="mb-4 text-xl md:text-2xl">
					Here are some great companies I had the honour to join for working on
					various interesting projects.
				</h2>
				<div>
					<div>
						<h2 className="text-xl font-bold">React Web Dev</h2>
						<span></span>
						<h2 className="font-medium text-md">Freelance</h2>
						<h2 className="font-medium text-md">From To</h2>
					</div>
					<div>
						<div className="h-10 border-l-2 border-black"></div>
						<h2 className="text-xl font-bold">
							Video Editor and Motion Graphics Artist
						</h2>
						<h2 className="font-medium text-md">94 Films</h2>
						<h2 className="font-medium text-md">From To</h2>
					</div>
					<div>
						<div className="h-10 border-l-2 border-black"></div>
						<h2 className="text-xl font-bold">Sr. Business Analyst</h2>
						<h2 className="font-medium text-md">MindScroll</h2>
						<h2 className="font-medium text-md">From To</h2>
					</div>
					<div>
						<div className="h-10 border-l-2 border-black"></div>
						<h2 className="text-xl font-bold">Business Analyst</h2>
						<h2 className="font-medium text-md">GyanDhan</h2>
						<h2 className="font-medium text-md">From To</h2>
					</div>
					<div>
						<div className="h-10 border-l-2 border-black"></div>
						<h2 className="text-xl font-bold">Computer Science Grad</h2>
						<h2 className="font-medium text-md">Thapar Unievrsity</h2>
						<h2 className="font-medium text-md">From To</h2>
					</div>
				</div>
			</div>
		</div>
	);
}

export default Experience;
